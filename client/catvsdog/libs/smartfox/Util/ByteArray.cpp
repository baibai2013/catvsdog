...
void ByteArray::Compress()
{
	...
	boost::shared_ptr<unsigned char> dataCompressed (new unsigned char[buffer->size()]);
	memset (dataCompressed.get(), 0x00, buffer->size());
...
{
	boost::shared_ptr<unsigned char> dataUncompressed (new unsigned char[65535]);
	memset (dataUncompressed.get(), 0x00, 65535);

	boost::shared_ptr<unsigned char> dataToUncompress (new unsigned char[buffer->size()]);
...
//    long int exponent = (hexNumber & 0x7f800000) >> 23;      
    ...
    {  
        ...
        total += (float)c * (float)pow(2.0, (double)power);
        power--;  
    }  
    total += 1.0;  
  
    returnedValue = sign * (float)pow(2.0, (double)exponent) * total;
...
//    long int exponent = (hexNumber & 0x7ff0000000000000) >> 52;      
    ...
    {  
        ...
        total += (double)c * (double)pow(2.0, (double)power);  
        power--;  
    }  
    total += 1.0;  
  
    returnedValue = sign * (double)pow(2.0, (double)exponent) * total;  
